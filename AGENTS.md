...
`engine/` is the shared, domain-agnostic runtime: `engine/agent/` (generation runtime, the Gemini adapter, prompt compiler/loader, tools, cost tracking, TUI helpers, batch orchestration), `engine/storage/` (the canonical `data/` layout, records, categories, dataset metadata, batch specs, run caches, materialization metadata, and search indexes), `engine/viewer/api/` (FastAPI) and `engine/viewer/web/` (React/TypeScript/Three.js viewer), `engine/cli/` (the `articraft` entry points), and `engine/articraft/` (config + value enums). `sdk/` and `sdk/_core/` define the articulated-object SDK layers (a top-level package because generated `model.py` does `import sdk`); `sdk/_docs/` and `sdk/_examples/` are agent-facing authoring assets. See `GENERATORS.md`. `tests/` mirrors the main packages with focused smoke/regression coverage.
...
## Configuration Tips
The engine loads `.env`. Set `GEMINI_API_KEYS` (comma-separated for rotation, or a single key), or use the Vertex backend (`GOOGLE_CLOUD_PROJECT`, `GOOGLE_APPLICATION_CREDENTIALS`, `GOOGLE_CLOUD_LOCATION`). `ARTICRAFT_MODEL` and `ARTICRAFT_THINKING_LEVEL` set the default generation model and thinking level; without those, `generate` defaults to `gemini-3.5-flash` with `--thinking-level high`. `ARTICRAFT_MAX_COST_USD` can set a default per-run budget.
