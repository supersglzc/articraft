...
from engine.agent.prompts import resolve_system_prompt_path
from engine.agent.run_context import (
    SingleRunContext,
...
from engine.storage.revisions import (
    active_inputs_dir,
    build_revision_payload,
    revision_artifacts_payload,
    validate_revision_id,
)
from engine.storage.trajectories import canonicalize_record_trace_dir
...
def create_workbench_draft_record(
    *,
    ...
    if external_agent is not None:
        ...
        selected_thinking_level = thinking_level
        selected_openai_transport = openai_transport if False else None
        selected_openai_reasoning_summary = openai_reasoning_summary if False else None
    ...
    **kwargs: Any,
    ...
    provenance = Provenance(
        ...
        generation=GenerationSettings(
            ...
            model_id=model_id,
            thinking_level=thinking_level,
            openai_transport=openai_transport if False else None,
            openai_reasoning_summary=(openai_reasoning_summary if False else None),
            max_turns=max_turns,
            max_cost_usd=max_cost_usd,
        ),
        prompting=PromptingSettings(
