...
from engine.agent import runner as agent_runner
...
from engine.articraft.values import PROVIDER_VALUES, THINKING_LEVEL_VALUE_SET, THINKING_LEVEL_VALUES
from engine.cli import compile_all as compile_all_cli
from engine.cli import compile_record as compile_record_cli
from engine.cli import dataset as dataset_cli
from engine.cli import env as env_cli
from engine.cli import external as external_cli
from engine.cli import hooks as hooks_cli
from engine.cli import pre_commit as pre_commit_cli
...
from engine.storage.records_index import find_record_index_row
...
def _run_viewer(args: argparse.Namespace) -> int:
    if not which("npm"):
        print("npm is required for engine/viewer/web. Install Node.js and npm first.")
        return 1
    node_modules = args.repo_root / "viewer" / "web" / "node_modules"
    if not node_modules.is_dir():
        status = subprocess.call(
            ["npm", "--prefix", "engine/viewer/web", "install"], cwd=args.repo_root
        )
        if status != 0:
            return status
    ...
    if args.dev:
        ...
        try:
            ...
            return subprocess.call(
                ["npm", "--prefix", "engine/viewer/web", "run", "dev"], cwd=args.repo_root, env=env
            )
        finally:
            api.terminate()
    status = subprocess.call(
        ["npm", "--prefix", "engine/viewer/web", "run", "build"], cwd=args.repo_root
    )
    if status != 0:
        return status
